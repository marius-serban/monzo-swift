...
class ClientGlobalTests : XCTestCase {
    ...
    func test_givenClientIdAndRedirectUriAndNonce_whenGeneratingAuthorizationUri_thenAuthorizationUriIsCorrect() {
        guard let uri = try? Client.authorizationUri(clientId: "aClientId", redirectUri: "http://host.com/?test=[]#fragment", nonce: "abc123") else { XCTFail(#function); return }
        
        XCTAssertEqual(uri.description, "https://auth.getmondo.co.uk/?client_id=aClientId&redirect_uri=http://host.com/?test=%5B%5D%23fragment&state=abc123")
    }
