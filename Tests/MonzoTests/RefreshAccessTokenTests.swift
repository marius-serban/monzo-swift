...
class RefreshAccessTokenTests : XCTestCase {
    
    func test_requestHasCorrectMethod() {
        let method = request(forClientAction: { sut in
            try sut.refreshAccessToken(refreshToken: "", clientId: "", clientSecret: "")
        }).method
    ...
    func test_givenRefreshParameters_thenTheRequestHasCorrectParametersInBody() {
        let body = requestBody(forClientAction: { sut in
            try sut.refreshAccessToken(refreshToken: ":/?#[]@!$&'()*+,;=🌎-._~ ", clientId: "a string with spaces", clientSecret: "")
        })
        
        XCTAssertNotNil(body?.range(of: "grant_type=refresh_token"))
        XCTAssertNotNil(body?.range(of: "client_id=a+string+with+spaces"))
        XCTAssertNotNil(body?.range(of: "client_secret="))
        XCTAssertNotNil(body?.range(of: "refresh_token=%3A%2F%3F%23%5B%5D%40%21%24%26%27%28%29%2A%2B%2C%3B%3D%F0%9F%8C%8E-._~+"))
    }
    ...
    func test_givenASucessfulResponse_thenCorrectCredentialsAreReturned() {
        ...
        do {
            let credentials = try sut.refreshAccessToken(refreshToken: "", clientId: "", clientSecret: "")
    ...
    func test_givenAnInvalidResponseStatus_thenResponseErrorIsThrown() {
        ...
        XCTAssertThrowsError(try sut.refreshAccessToken(refreshToken: "", clientId: "", clientSecret: ""), #function) { error in
    ...
    static var allTests : [(String, (RefreshAccessTokenTests) -> () throws -> Void)] {
        return [
            ...
            ("test_givenRefreshParameters_thenTheRequestHasCorrectParametersInBody", test_givenRefreshParameters_thenTheRequestHasCorrectParametersInBody),
